...
def generate_launch_description():
    ...
    return LaunchDescription([
        DeclareLaunchArgument(
            ...
            default_value="true",
            description=
            "Flag to publish a standalone azure_description instead of the default robot_description parameter."),
        ##############################################
        DeclareLaunchArgument('depth_enabled', default_value="true", description="Enable or disable the depth camera"),
        DeclareLaunchArgument(
            'depth_mode',
            default_value="WFOV_UNBINNED",
            description=
            "Set the depth camera mode, which affects FOV, depth range, and camera resolution. See Azure Kinect documentation for full details. Valid options: NFOV_UNBINNED, NFOV_2X2BINNED, WFOV_UNBINNED, WFOV_2X2BINNED, and PASSIVE_IR"
        ),
        DeclareLaunchArgument(
            'depth_unit',
            default_value="16UC1",
            description=
            'Depth distance units. Options are: "32FC1" (32 bit float metre) or "16UC1" (16 bit integer millimetre)'),
        DeclareLaunchArgument('color_enabled', default_value="true", description="Enable or disable the color camera"),
        DeclareLaunchArgument('color_format',
                              default_value="bgra",
        ...
        DeclareLaunchArgument(
            ...
            default_value="1536P",
            description=
            "Resolution at which to run the color camera. Valid options: 720P, 1080P, 1440P, 1536P, 2160P, 3072P"),
        DeclareLaunchArgument('fps',
                              default_value="5",
                              description="FPS to run both cameras at. Valid options are 5, 15, and 30"),
        DeclareLaunchArgument('point_cloud',
                              default_value="true",
        ...
        DeclareLaunchArgument(
            ...
            default_value="false",
            description=
            "Whether the RGB pointcloud is rendered in the depth frame (true) or RGB frame (false). Will either match the resolution of the depth camera (true) or the RGB camera (false)."
        ),
        ...
        DeclareLaunchArgument('sensor_sn',
                              default_value="",
        ...
        DeclareLaunchArgument(
            ...
            default_value="",
            description=
            "Absolute path to a mkv recording file which will be used with the playback api instead of opening a device"
        ),
        DeclareLaunchArgument(
        ...
        DeclareLaunchArgument('body_tracking_enabled',
                              default_value="false",
                              description="If set to true the joint positions will be published as marker arrays"),
        DeclareLaunchArgument('body_tracking_smoothing_factor',
                              default_value="0.0",
        ...
        DeclareLaunchArgument(
            ...
            default_value="1.0",
            description=
            "Scaling factor to apply when converting IR to mono8 (see rescale_ir_to_mono8). If using illumination, use the value 0.5-1. If using passive IR, use 10."
        ),
        DeclareLaunchArgument(
            'imu_rate_target',
            default_value="0",
            description="Desired output rate of IMU messages. Set to 0 (default) for full rate (1.6 kHz)."),
        DeclareLaunchArgument('wired_sync_mode',
                              default_value="0",
                              description="Wired sync mode. 0: OFF, 1: MASTER, 2: SUBORDINATE."),
        DeclareLaunchArgument('subordinate_delay_off_master_usec',
                              default_value="0",
        ...
        launch_ros.actions.Node(package='robot_state_publisher',
                                ...
                                condition=conditions.IfCondition(
                                    launch.substitutions.LaunchConfiguration("overwrite_robot_description"))),
        launch_ros.actions.Node(package='joint_state_publisher',
        ...
        launch_ros.actions.Node(package='robot_state_publisher',
                                ...
                                condition=conditions.UnlessCondition(
                                    launch.substitutions.LaunchConfiguration("overwrite_robot_description"))),
        launch_ros.actions.Node(package='joint_state_publisher',
                                executable='joint_state_publisher',
                                name='joint_state_publisher',
